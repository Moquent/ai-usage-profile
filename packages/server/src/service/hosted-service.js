...
export async function createHostedService({
  ...
  const store = repository ?? new ProfileRepository({ filename: databasePath, now });
  ...
  async function requirePublisher(request, reply) {
    const profile = store.getProfileById(request.params.id);
  ...
  function writeSnapshot(request, reply, profile) {
    ...
    const existing = store.getSnapshot(profile.id);
    ...
    const result = store.saveSnapshot(profile.id, envelope);
    return {
  ...
  app.patch("/v1/profiles/:id", {
    ...
    const existing = store.getProfileById(request.params.id);
    ...
    const profile = store.updateProfile(existing.id, card);
    return publicProfile(request, profile, publicBaseUrl);
  });
  ...
  app.post("/v1/profiles/:id/token", {
    ...
    const profile = store.updatePublishToken(request.params.id, hashCredential(publishToken));
    if (!profile) return reply.code(404).send(errorPayload(request, "not_found", "Profile not found"));
    return { publishToken };
  });
  ...
  app.delete("/v1/profiles/:id", {
    ...
    if (!store.deleteProfile(request.params.id)) {
  ...
  app.put("/v1/me/snapshot", {
    ...
    let profile;
    try {
      profile = store.upsertGitHubProfile({
        id: createProfileId(),
  ...
  app.get("/v1/me/status", {
    ...
    const profile = store.getProfileByGithubUserId(request.githubUser.id);
    if (!profile) {
      return {
    ...
    return {
      ...
      ...snapshotStatus(profile, store.getSnapshot(profile.id), cardUrl(request, profile, publicBaseUrl)),
    };
  });
  ...
  app.get("/v1/profiles/:id/status", {
    ...
    return snapshotStatus(
      request.profile,
      store.getSnapshot(request.profile.id),
  ...
  app.get("/u/:slug/card.svg", {
    ...
    const profile = store.getProfileBySlug(request.params.slug);
    if (!profile) return reply.code(404).type("text/plain").send("Profile not found\n");
    const stored = store.getSnapshot(profile.id);

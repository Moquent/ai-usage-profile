...
import { publicCardUrl, publishProviderSnapshot, resolvePublicOrigin, verifyGitHubUser } from "@ai-usage/shared";
...
export async function loginWithDeviceFlow({
  ...
  if (!clientId) {
    throw new Error("AI_USAGE_GITHUB_CLIENT_ID is required for GitHub device login");
  }
...
async function resolveGitHubAuth({
  ...
  const githubToken = await loginWithDeviceFlow({
    clientId: clientId ?? env.AI_USAGE_GITHUB_CLIENT_ID,
    onVerification,
    createAuth,
  });
